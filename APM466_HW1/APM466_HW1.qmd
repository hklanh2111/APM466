...
# -------------------------
# Lecture simplifications: ACT/365, semiannual coupons
# -------------------------
...
```{r}
# ============================================================
# Q4(a): YTM (continuous comp) + superimposed 0–5y curves
# Interpolation technique: linear interpolation between observed maturities
# ============================================================
...
ggplot() +
  ...
  labs(
    title = "Yield Curves (Continuous-Compounded YTM), 0–5 Years",
...
# ============================================================
# ============================================================
# CHUNK B (COMPLETE + FIXED) — SPOT CURVE r(t) with DOC-STYLE X-AXIS (MATURITY DATE)
# 10 daily spot curves superimposed
#
# Does NOT assume tau_years exists; computes it as (maturity - settle)/365
# ============================================================
...
```{r}
# ============================================================
# CHUNK C (COMPLETE + FIXED) — 1-YEAR FORWARD CURVE with DOC-STYLE X-AXIS (DATE)
# Uses spot_points created in CHUNK B.
# ============================================================
...
# Interpolate spot r(1)..r(5) per day
...
print(fwd_1yr %>% arrange(settle, endT), n = Inf)
...
make_latex_eigen_table <- function(CovMat, caption_text) {

  eg <- eigen(as.matrix(CovMat))
  lam <- eg$values
  V   <- eg$vectors

  fmt_lam <- function(x) sprintf("%.6e", x)
  fmt_v   <- function(x) sprintf("% .4f", x)

  lam_row <- paste(sapply(lam, fmt_lam), collapse = " & ")
  V_rows  <- apply(V, 1, function(row) paste(sapply(row, fmt_v), collapse = " & "))

  cat(paste0(
"\\[
\\begin{array}{c|", paste(rep("c", ncol(V)), collapse=""), "}
\\lambda & ", lam_row, " \\\\
\\hline
\\vec{v} &
\\begin{array}{", paste(rep("c", ncol(V)), collapse=""), "}
", paste(V_rows, collapse=" \\\\\n"), "
\\end{array}
\\end{array}
\\]
\n\n\\textit{", caption_text, "}\n\n"
...
cat("The eigenvalues and eigenvectors for the log-returns of yield covariance matrix are:\n\n")
make_latex_eigen_table(Cov_Yield, "Table: eigen of yield rate")

cat("The eigenvalues and eigenvectors for the log-returns of forward covariance matrix are:\n\n")
make_latex_eigen_table(Cov_Forward, "Table: eigen of forward rate")
